...
from ml.deeplearning.nwjc2gnews2vec_sigmoid5_softmax import NWJC2GNEWS2VECSigmoid5SOFTMAX
...
def main(start_k, end_k, start_epoch, end_epoch, n_in, n_mid, batchsize, gpu, window_size, patience=0):
    ...
    """
    print("-------------------------------------")
    print("exec_file: ja_amazon_nwjc2gnews2vec_sigmoid5_softmax.py")
    ...
    # 実験ディレクトリ
    experiment_dir = constants.AMAZON_DIR + "experiment/ja/nwjc2gnews2vec_sigmoid5_softmax/window" + str(window_size) + "/"
    experiment_majority_dir = constants.AMAZON_DIR + "experiment/ja/nwjc2gnews2vec_sigmoid5_majority/window" + str(window_size) + "/"
    ...
    for k in range(start_k, end_k+1):
        print(str(k) + " / 5 分割目")
        # ネットワークインスタンス作成
        net = NWJC2GNEWS2VECSigmoid5SOFTMAX(n_in, n_mid, batchsize, gpu, window_size)
...
if __name__ == '__main__':
    # 引数パース
    parser = argparse.ArgumentParser(description='ja_Amazonコーパスについて、nwjc2gnews2vecとsigmoid5_softmaxによって5値分類する')

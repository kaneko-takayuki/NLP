...
def main(start_k, end_k, start_epoch, end_epoch, n_in, n_mid, batchsize, gpu, window_size, completion):
    ...
    for k in range(start_k, end_k+1):
        ...
        for epoch in range(start_epoch, end_epoch+1):
            # 英語データを用いて学習させたモデルを読み込む
            model_file = constants.SIGMOID5_DIR + "window" + str(window_size) + "/" + str(completion) + "/" \
                         + "model/cross_validation" + str(k) + "/" \
...
if __name__ == '__main__':
    # 引数パース
    parser = argparse.ArgumentParser(description='Amazonコーパスについて、英語から日本語へのファインチューニングを行い、sigmoid5によって5値分類する')
